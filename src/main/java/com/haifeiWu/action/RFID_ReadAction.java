	...
	public String readRFID() throws IOException {
		...
		if (RFID_ReadAction.isEmpty == true) {// 如果房间为空
			...
			if (roomId == 1) {// 如果房间号为1，则……
				...
				json2 = json2.substring(0, 31) + "[" + json2.substring(31, 63)
						+ "]" + json2.substring(63, 79);// 封装json字符串，使用fastjson
				// 激活第一个摄像头
				String str1 = HttpRequest.sendOkMCVPost(
						PropertiesReadUtils.getString("SetSplitType"), json2);
				...
				if (person.getIdentifyCard_Number() != null) {// 开始录制视频
					...
					str1 = HttpRequest.sendOkMCVPost(
							PropertiesReadUtils.getString("StartRecording"),
							json);
			...
			if (wristId.equals("FFFFFFFF") && roomId == 4) {// 当嫌疑出区后，自动停止录播系统录像
				...
				String str = HttpRequest.sendOkMCVPost(
						PropertiesReadUtils.getString("StopRecording"), null);
			...
			if (!oldDeviceId.equals(deviceId)) {// 如果设备号发生改变
				...
				if (isRecording == true) {
					...
					// 发送暂停录像指令
					String str1 = HttpRequest.sendOkMCVPost(
							PropertiesReadUtils.getString("PauseRecording"),
							null);
					// 打印返回的状态码
					...
					isRecording = true;
					String str1 = HttpRequest.sendOkMCVPost(
							PropertiesReadUtils.getString("RestartRecording"),
							null);
					// 打印返回的状态码
