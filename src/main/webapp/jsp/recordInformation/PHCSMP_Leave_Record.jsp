					...
					<div id="state" class="col-lg-12 col-md-10 col-sm-10"
						...
						<c:if test="${!empty personal_Check }">
							<script type="text/javascript">
								$(document).ready(
										function() {
											$("#identityImg1").attr("src",
													"images/fgreen_03.png");
				...
				<div class="row" style="margin-bottom: 10px;">
					...
					<div class="pic col-lg-4 col-md-4 col-sm-4 col-xs-4"
						...
						<img id="img_1" src="${suspectInfor.frontal_Photo}"
							style="border: 1px solid #ccc" /> <img id="img_2"
							src="${suspectInfor.sideWays_Photo}"
							style="border: 1px solid #ccc" />
						<p class="date_pic col-lg-6 col-md-6 col-sm-6"
							style="margin-left: 0px;">${nEntryTime }嫌疑人入区登记照片</p>
					</div>
					<div class="col-lg-8 col-md-8 col-sm-8 col-xs-8"
						style="margin-top: 30px;">
						<hr
							style="width: 96%; border: 0.2px solid #389ac7; padding: 0px; margin-top: 1%; margin-left: -10%;" />
						<table class="Message col-lg-12 col-md-12 col-sm-12 col-xs-12"
							style="margin-bottom: 15px;">
							<tr style="padding: 0px;">
								<!--图片引入-->
								<td rowspan="5"><img id="pic"
									src="${suspectInfor.identityCard_Photo }"
									style="border: 1px solid #ccc; width: 100px; height: 108px; position: relative; top: 0px;" />
								...
								<td colspan="2">姓名: <input type="text" name="suspect_Name"
									value="${suspectInfor.suspect_Name }"
									style="font-size: 12px; margin-left: 10px;" />
								</td>
							</tr>
							<!--第二行 性别 民族-->
							<tr>
								<td>性别： <input style="font-size: 12px; margin-left: 10px;"
									type="text" name="sex" value="${suspectInfor.sex }" />
								</td>
								<td style="margin-left: 10px;">民族：<input type="text"
									name="nation" value="汉"
									style="font-size: 12px; margin-left: 10px;" /></td>
							</tr>
							<!--第三行 出生-->
							<tr>
								<td colspan="8">出生日期： <input type="text" name="birthday"
									value="${suspectInfor.birthday }" />
								</td>
							</tr>
							<!--第四行身份证住址-->
							<tr>
								<td colspan="8">家庭住址：<textarea name="address" rows="4"
										cols="45" col-md-offset-2>${suspectInfor.address }</textarea>
								</td>
							</tr>
							<tr>
								<td colspan="10">身份证号码: <input type="text"
									name="identifyCard_Number"
						...
						<hr
							style="width: 96%; border: 0.2px solid #389ac7; padding: 0px; margin-top: 13%; margin-left: -10%;" />
					</div>
				</div>
			</div>
			<!-- 完整性检查表 -->
			<div class="container">
				<div class="row">
					<h4 id="leaveReco"
						...
						完整性检查
						<!-- <span class="col-lg-12 col-md-12 col-sm-12">填写完整度0%</span> -->
					</h4>
				</div>
			</div>
			<div class="msg"
				style="color: red; margin-top: 20px; margin-left: 25px;">${sb }</div>
								...
								<tr>
									...
									<c:if test="${! empty a.activity_Record }">
										<td style="text-align: left; padding-left: 30px;"
											class="activity">活动内容：${a.activity_Record }</td>
									</c:if>
									<c:if test="${empty a.activity_Record }">
										<td style="text-align: left; padding-left: 30px;"
											class="activity">活动内容：空</td>
									</c:if>
								</tr>
							</c:forEach>
						</c:if>
		...
		<form class="transient"
			...
			<div class="container">
						...
						<c:if test="${!empty temporaryLeave }">
							<tr>
								...
								<td style="padding: 6px 0 6px 0"><input name="staff_ID"
									class="staff_ID" type="text"
									style="border-radius: 6px; border: 1px solid #ccc; padding: 8px 0 8px 0;"
				...
				<div class="row" style="margin-top: 30px; width: 1000px;">
					...
					<div
						style="float: left; width: 460px; margin-left: -160px; margin-top: -10px;">
				...
				<div class="row ">
					<table class="final_Leave col-lg-12 col-md-12 col-sm-12">
						...
						<tr>
							...
							<td><c:forEach items="${keepingWay }" var="v"
									varStatus="status">
									<input type="radio" name="belongingS_Treatment_Method"
										value="${v.keeping_Name }" class="checkRadio">${v.keeping_Name }
							</c:forEach></td>
						</tr>
						<tr>
							<td style="color: #000 !important;">未反还物品情况记载:</td>
							<td style="padding: 8px 0;"><c:forEach
									items="${treatmentMethod }" var="t" varStatus="status">
									<input type="radio" name="belongingS_Treatment_Record"
										value="${t.treatment_Name }" class="checkRadio1">${t.treatment_Name }
							</c:forEach></td>
						</tr>
						<tr>
							<td>领取人签名:</td>
							<td style="padding: 8px 0 8px 0"><input
								style="border-radius: 6px; border: 1px solid #ccc; padding: 8px 0 8px 0;"
								type="text" name="recipient_Person"
								value="${PHCSMP_Leave_Record.recipient_Person }" /></td>
						</tr>
						<tr>
							<td>领取人身份证号码:</td>
							<td style="padding: 8px 0 8px 0"><input
								style="margin-left: 60px; border-radius: 6px; border: 1px solid #ccc; padding: 8px 0 8px 0;"
								type="text" name="recipient_Person_Number"
								value="${recipient_Person_Number }" class="identity" /> <input
								type="button" value="读卡" class="subs" /></td>
						</tr>
					</table>
				</div>
				<div class="row" style="margin-top: 30px; width: 1000px;">
					<div style="float: left; width: 400px; margin-left: 150px">
						<p id="signature">
							管理员:<input type="text" name="staff_ID"
								value="${PHCSMP_Leave_Record.staff_ID }" />
						</p>
					</div>
					<div
						style="float: left; width: 460px; margin-left: -160px; margin-top: -10px;">
